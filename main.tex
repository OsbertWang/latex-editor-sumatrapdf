...
这里的 \texttt{?c:am.}、\texttt{@}、\texttt{\%\%} 均是 \TeX Studio 自己定义的有特殊含义的字符.
接下来,
...
同样先介绍命令行
\begin{lstlisting}
  <SumatraPDFROOT>\SumatraPDF.exe -reuse-instance -forward-search main.tex 4 -inverse-search "\"<VSCodeROOT>\Code.exe\" -r -g \"%f:%l\"" main.pdf
\end{lstlisting}
...
并且我们比命令行时多添加了一段 \texttt{cli.js} 相关的代码.
下面略加解释以上设置的用意.
